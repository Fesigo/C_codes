...
void altera_prop(prop *p, int tam);
int cadastro_imovel(imovel *im, int qim, prop *p, int qreg);
...
int main()
{
	...
	do
	{
		system("cls");
//		printf("\nsize prop: %i\n\n", sizeof(prop));
//		printf("\nsize prop - info_casa: %i\n\n", sizeof(prop) - sizeof(info_casa));
//		printf("\nsize int: %i\n\n", sizeof(int));
...
//				(p->casa + p->qtd_casa)->num_casa = -1;
//				(p->casa + p->qtd_casa)->status_casa = 'L';
//				fseek(fptr, i*sizeof(prop), 0);

//				fwrite(p, sizeof(prop), 1, fptr);
//				altera_prop(p+i, qreg);

				FILE *fptr2=NULL;
				
				if((fptr2=(fopen("proprietario.bin", "rb+")))==NULL)
					printf("\nErro na alteracao\n");
				else
				{
