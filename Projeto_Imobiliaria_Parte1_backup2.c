...
typedef struct prop{
	...
	endereco end_prop;
	int qtd_casa;
	info_casa casa[5];  //Obs.: (p->casa+i)->status_casa
}prop;
...
int verifica_prop();
int verifica_im();
void cadastro_prop(prop *p, int qreg);
void grava_prop(prop *p);
void mostra_prop(prop *p, int qreg);
void mostra_prop_parcial(prop *p, int qreg);
void cadastro_imovel(imovel *im, int qim);
void grava_im(imovel *im);
...
int main()
{
	...
	do
	{
		...
		{
			cadastro_imovel(im, qim+1); //Cadastra os imoveis
			qim++;
		}
		else if(op==5)
		...
		for(i=0; i<qreg; i++)
		{
			...
			if(p->qtd_casa > 0)
			{
				for(j=0; j<(p->qtd_casa); j++)
					printf("\nCasa - %i\nN: %i\nStatus: %s\n", j+1/*, (p->casa)->num_casa, (p->casa)->status_casa*/); //(p->casa+i)->status_casa
			}
		}
		fclose(fptr);
	}
...
void cadastro_imovel(imovel *im, int qim) //Cadastro de imóveis
{
	im->reg_imov = qim;
	...
	else
	{
		...
		if(op==1)
		{
			...
			for(i=0; i<qreg; i++)
			{
				...
				if(im->status.sigla == 'L' && im->area >= a)
				{
					vdd++;
		...
		else if(op==2)
		{
			...
			for(i=0; i<qreg; i++)
			{
				...
				if(im->status.sigla == 'L' && im->quartos == q)
				{
					vdd++;
...
//				maiuscula(im->end_imov.bairro);
				...
				if(im->status.sigla == 'L' && strcmp(b, im->end_imov.bairro)==0)
				{
					vdd++;
		...
		if(vdd==0)
			printf("Na ha imoveis com os requisitos digitados\n");
	}
}
