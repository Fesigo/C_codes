...
void altera_prop(prop *p, int qprop, int nreg, int op); //Alteração no cadastro dos proprietários, adicionando o locatário
int busca_im(imovel *im, int qim, int nreg); //Faz a busca do imóvel que passará pela alteração
void altera_im(imovel *im, int qim, int pos, locatario *l, int op); //Alteração no cadastro dos imóveis, adicionando o locatário
...
void altera_loc(locatario *l, int qloc, int pos);
...
{
	FILE *fptr=NULL;
	int i, vdd=0, dia, mes, ano, d, m, a, cont=0, data1, data2, pos;
	char *aux = NULL;
	...
	else
	{
		...
		for(i=0; i<qloc; i++)
		{
			...
			if(strcmp(aux, l->CPF)==0)
			{
				...
				data1 = dia + mes*100 + ano*10000; //data atual
				data2 =  d + m*100 + a*10000; //data do cancelamento do contrato
				
				pos = busca_im(im, qim, l->reg_imov);
				
				if(data1 < data2) //data do térmicno do contrato é depois da data atual
				...
				else //data atual é depois da data de término do contrato
...
void altera_im(imovel *im, int qim, int pos, locatario *l, int op) //Alteração no cadastro dos imóveis, adicionando o locatário
{
	...
	else
	{
		...
		if(op==0) //fazer outras opções para a rotina de altereções
		{
